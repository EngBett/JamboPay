...
namespace JamboPay.Controllers
{
    ...
    public class TransactionController : Controller
    {
        ...
        public async Task<IActionResult> AddTransaction(TransactionViewModel model)
        {
            ...
            var transaction = _transactionRepository.AddTransaction(new Transaction{ServiceId = model.ServiceId,Cost = model.Cost,ApplicationUserId = model.ApplicationUserId});
            if (await _transactionRepository.SaveChangesAsync())
            {

                var service = await _serviceRepository.FetchService(model.ServiceId);
                var network = _userNetworkRepository.GetUserNetwork(model.ApplicationUserId);
                
                _commissionRepository.AddCommision(new Commission{ApplicationUserId = network.Network.ApplicationUserId,Amount = service.CommissionPercentage * model.Cost, TransactionId = transaction.Id});
